...
function isRenkoRecord(value = {}) {
  ...
  const raw = upper(
    ...
    value?.positionSnapshot?.strategyLane || value?.positionSnapshot?.entryStrategy ||
...
function renkoPerformanceSummary() {
  ...
  for (const record of Object.values(state.records || {})) {
    ...
    const realized = finite(record?.realizedPnl, finite(record?.accounting?.realizedPnl, 0));
    bucket.netPnl += net;
    bucket.commission += commission;
    bucket.grossRealizedPnl += realized;
    if (net > 1e-9) bucket.wins++;
