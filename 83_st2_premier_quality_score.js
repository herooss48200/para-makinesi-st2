...
const VERSION = 'v6.14.0-R26-CORE-PREMIER-QUALITY-SCORE';
...
const WEIGHT_KEYS = Object.freeze([
  'historicalPf', 'historicalExpectancy', 'liveForm', 'entryEvolution', 'sampleConfidence'
]);
const DEFAULT_WEIGHTS = Object.freeze({
  historicalPf: 21,
  historicalExpectancy: 20,
  liveForm: 24,
  entryEvolution: 18,
  sampleConfidence: 17
});
...
function validWeights(raw = {}) {
  // R26: eski 6-bileşenli kalibrasyondan yalnız çekirdek ağırlıkları al ve 100'e normalize et.
  const vals = {};
  ...
  const out = {};
  for (const key of WEIGHT_KEYS) out[key] = vals[key] * 100 / sum;
  return out;
}
...
function scoreEvidence(input = {}) {
  ...
  const components = {
    ...
    sampleConfidence: clamp(
      sampleScore(historicalN, 18) * 0.60 +
      sampleScore(Math.max(liveN, entryN), 12) * 0.40
    )
  };
...
function evaluate(input = {}) {
  ...
  return {
    ...
    liveWindow: policy.liveWindow, selected, executionMode: selected ? 'PREMIER' : 'REJECT',
...
function applyLabReview(result = {}, review = null) {
  ...
  return {
    ...
    score,
    selected,
    executionMode: selected ? 'PREMIER' : 'REJECT',
    reason,
...
function componentText(result = {}) {
  ...
  return `Puanlar: PF ${n(c.historicalPf).toFixed(1)}/100 | Exp ${n(c.historicalExpectancy).toFixed(1)}/100 | Canlı ${n(c.liveForm).toFixed(1)}/100 | Entry ${n(c.entryEvolution).toFixed(1)}/100 | Örnek ${n(c.sampleConfidence).toFixed(1)}/100`;
}

function weightedComponentText(result = {}) {
  const c = result.components || {};
  const w = result.weights || activePolicy().weights;
  const names = { historicalPf: 'PF', historicalExpectancy: 'Exp', liveForm: 'Canlı', entryEvolution: 'Entry', sampleConfidence: 'Örnek' };
...
module.exports = {
  VERSION, WEIGHTS, DEFAULT_WEIGHTS, WEIGHT_KEYS, ENTRY_STATE, CALIBRATION_FILE,
  ...
  entryEvolutionEvidence, scoreEvidence, quantile, rank, evaluate, applyLabReview,
