...
function candles(tfMs,count,base=100){
  ...
  return Array.from({length:count},(_,i)=>({openTime:start+i*tfMs,closeTime:start+(i+1)*tfMs-1,open:String(base),high:String(base+1),low:String(base-1),close:String(base+.5),volume:'1'}));
}
