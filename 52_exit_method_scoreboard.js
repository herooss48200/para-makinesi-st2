...
function telegramLine(s){if(!s)return'';const pf=Number.isFinite(s.pf)?s.pf.toFixed(2):'∞';return `\n🎯 Exit Metodu: <b>${s.label}</b>\n📒 Metot Çetelesi: Açılan ${s.opened} | Kapalı ${s.closed} | TP ${s.tp} SL ${s.sl} BE ${s.be} | Başarı %${s.success.toFixed(1)} | Net ${s.net>=0?'+':''}${s.net.toFixed(4)} | PF ${pf} | Exp ${s.expectancy>=0?'+':''}${s.expectancy.toFixed(4)}`;}
