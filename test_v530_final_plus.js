...
assert.equal(life.STOP_MIN_SAMPLES(),5,'LAB Stop minimum 5 olmalı');
assert.equal(life.BE_MIN_SAMPLES(),5,'LAB BE+ minimum 5 olmalı');
...
const c={};for(let i=0;i<5;i++){c.samples=(c.samples||0)+1;c.net=(c.net||0)+.1;c.grossProfit=(c.grossProfit||0)+.1;c.grossLoss=0}
const pick=life.champion({'1.00':c},'1.50');assert(pick.ready,'5 pozitif kapanışta Stop adayı hazır olmalı');
...
assert(src.includes("premierTrack: TRACK.REVERSE, upperLayerIncluded: false") || src.includes("league: 'NEGATIVE', premierTrack: TRACK.NEGATIVE, upperLayerIncluded: false"),'Negative ters gölge Premier kasasına dahil olmamalı');
