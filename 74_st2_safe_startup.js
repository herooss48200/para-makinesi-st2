...
function verifyOrThrow(){
  if(ayarlar.entryStrategyMode!=='ST2_RENKO') return {ok:true,mode:'ST1'};
  ...
  if(summary.health?.state==='CORRUPT' || summary.health?.ledger==='CORRUPT') throw new Error('ST2_SAFE_STARTUP: Entry Evolution state/ledger bozuk; Trade Engine başlatılmadı.');
