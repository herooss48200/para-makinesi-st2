...
const VERSION = 'v3.6.5-EXIT-EVOLUTION-DNA-SCOREBOARD';
...
function ensureBucket(map, key, label) {
  if (!map[key]) map[key] = { key, label: label || key, samples: 0, netUsdt: 0, actualNetUsdt: 0, deltaUsdt: 0, winsVsActual: 0, lossesVsActual: 0, profitableTrades: 0, losingTrades: 0, grossProfitUsdt: 0, grossLossUsdt: 0 };
  return map[key];
}
...
function buildModel(o) {
  const algorithmRanking = Object.values(o.byAlgorithm).map(bucketModel).sort((a, b) => b.deltaUsdt - a.deltaUsdt || b.samples - a.samples);
  const dna = Object.values(o.bySignature).map(s => {
    const ranking = Object.values(s.algorithms || {}).map(bucketModel).filter(x => x.key !== 'ACTUAL').sort((a, b) => b.deltaUsdt - a.deltaUsdt || b.samples - a.samples);
    return { key: s.key, label: s.label, samples: s.samples, confidence: s.samples >= 3 ? 'GELISEN' : 'YETERSIZ_ORNEK', bestExit: ranking[0] || null, ranking };
  }).sort((a, b) => b.samples - a.samples);
  const systemComparison = { actualNetUsdt: round(o.actualTotalNetUsdt, 4), oracleBestNetUsdt: round(o.oracleBestTotalNetUsdt, 4), potentialDeltaUsdt: round(o.oraclePotentialDeltaUsdt, 4), improvementPct: o.actualTotalNetUsdt !== 0 ? round((o.oraclePotentialDeltaUsdt / Math.abs(o.actualTotalNetUsdt)) * 100, 1) : 0 };
  return { version: VERSION, createdAt: new Date().toISOString(), dataPolicy: 'Tarihsel replay kayıtları trade ID bazında tekilleştirilerek v3.6.5 DNA Scoreboard şemasına taşındı.', totalTrades: o.totalTrades, systemComparison, algorithmRanking, dna, missedOpportunityDna: dna.filter(x => x.bestExit).sort((a, b) => num(b.bestExit.deltaUsdt) - num(a.bestExit.deltaUsdt)).slice(0, 10), timeBehavior: [], last10: o.last10, migration: o.migration, pendingModels: ['ATR_TRAILING','TREND_EXIT','DYNAMIC_EXIT','HYBRID_EXIT','ALTERNATIVE_LADDER'] };
}
function main() {
