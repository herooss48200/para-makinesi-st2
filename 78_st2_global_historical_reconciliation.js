'use strict';
/** AGROS ST2 v6.1.0 — Global Historical Learning & Intelligence Reconciliation. Shadow-only. */
...
function finish(m){const d=m.wins+m.losses;return{...m,wr:d?m.wins/d*100:0,pf:m.grossLoss>0?m.grossProfit/m.grossLoss:(m.grossProfit>0?999:0),expectancy:m.n?m.net/m.n:0,avgWin:m.wins?m.grossProfit/m.wins:0,avgLoss:m.losses?-(m.grossLoss/m.losses):0,mfe:m.n?n(m.mfeSum)/m.n:0,mae:m.n?n(m.maeSum)/m.n:0}}
function add(m,pnl,extra={}){m.n++;m.net+=n(pnl);if(pnl>1e-9){m.wins++;m.grossProfit+=pnl}else if(pnl<-1e-9){m.losses++;m.grossLoss+=Math.abs(pnl)}else m.be++;m.mfeSum=n(m.mfeSum)+n(extra.mfe);m.maeSum=n(m.maeSum)+n(extra.mae)}
function actualNet(row){return winning.actualNet(row)}
function readJsonl(file,type){if(!fs.existsSync(file))return[];const out=[],seen=new Set();for(const line of fs.readFileSync(file,'utf8').split(/\r?\n/)){if(!line.trim())continue;try{const x=JSON.parse(line);if(type&&x.type!==type)continue;const id=x.closeId||x.tradeId||x.signalId||crypto.createHash('sha256').update(line).digest('hex');if(seen.has(id))continue;seen.add(id);out.push({...x,closeId:id})}catch(_){}}return out}
