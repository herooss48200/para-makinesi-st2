...
fs.writeFileSync(
  path.join(tmp, 'st2-historical-training.json'),
  JSON.stringify({ profiles: {} })
);
fs.writeFileSync(
  path.join(tmp, 'st2-historical-training-ledger.jsonl'),
  ''
);
...
const reg=intel.registry();assert.equal(reg.policy.shadowOnly,true);assert.equal(reg.profiles.length,2);
const p=reg.profiles.find(x=>x.context.pattern==='GGGG');assert(p);assert.equal(p.decision.brick,0.50);assert(p.confidence.score>=0&&p.confidence.score<=100);assert(p.nearest.length===1);assert(p.nearest[0].similarity>0);assert(p.expectation.liveWeight>0);assert(['GUCLENIYOR','ZAYIFLIYOR','YATAY','YETERSIZ_VERI'].includes(p.evolution.trend));
const tg=intel.telegram();assert(tg.includes('ADAPTIVE DNA INTELLIGENCE'));assert(tg.includes('Trade Engine') && tg.includes('değişmedi'));
console.log('âœ… v6.0.0 Adaptive DNA Intelligence passed | registry, confidence, similarity, expectation, evolution, shadow safety');
