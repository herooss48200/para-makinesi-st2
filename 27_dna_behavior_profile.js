/**
 * AGROS v3.8.0 - DNA BEHAVIOR PROFILE
 ...
 */
const VERSION = 'v3.8.0-DNA-BEHAVIOR-PROFILE';
...
function ladderCharacter(l){return l?.character||'VERI_BIRIKIYOR';}
function riskLevel(p,t,tr,v,l){
  ...
  if(l?.ready&&l.character==='KADEME_SONRASI_GERI_VEREN')return'YUKSEK';
...
function narrative(profile){
  ...
  const parts=[];const t=profile.time,p=profile.profit,tr=profile.trend,v=profile.volatility,l=profile.ladder;
  ...
  if(l.character==='ISTIKRARLI_TIRMANAN')parts.push('kademeleri istikrarlı tırmanan');
  else if(l.character==='KADEME_SONRASI_GERI_VEREN')parts.push('kademe sonrası geri dönüşü yüksek');
  else if(l.character==='UZUN_KADEME_KOSUCUSU')parts.push('yüksek kademelere taşıyabilen');
  else if(l.character==='ERKEN_SONEN')parts.push('erken kademelerde sönen');
...
function buildProfile(profit,time,trend,volatility,ladder,options={}){
  ...
  if(ladder)available.push(num(ladder.qualifiedSamples,ladder.samples));
  const samples=Math.min(...available.filter(x=>Number.isFinite(x)));
  const ready=!!profit?.ready&&!!time?.ready&&(!trend||!!trend.ready)&&(!volatility||!!volatility.ready)&&(!ladder||!!ladder.ready)&&samples>=minSample;
  const profile={
    key:profit?.key||time?.key||trend?.key||volatility?.key||ladder?.key||'SIGNATURE_YOK',
    label:profit?.label||time?.label||trend?.label||volatility?.label||ladder?.label||profit?.key||time?.key||'SIGNATURE_YOK',
    ...
    ladder:{character:ladderCharacter(ladder),averageMaxStage:round(ladder?.averageMaxStage,2),highestReliableStage:num(ladder?.highestReliableStage),averageStageChanges:round(ladder?.averageStageChanges,2),safestProfitStage:ladder?.safestProfitStage||null}
  };
  profile.risk=riskLevel(profit,time,trend,volatility,ladder);profile.summary=narrative(profile);return profile;
}
function buildModel(profitModel,timeModel,trendModel,volatilityModel,ladderModel,options={}){
  // Eski üç argümanlı çağrılarla geriye uyumluluk.
  if(trendModel&&!Array.isArray(trendModel.dna)&&(!volatilityModel||!Array.isArray(volatilityModel.dna))){options=trendModel;trendModel=null;volatilityModel=null;ladderModel=null;}
  if(ladderModel&&!Array.isArray(ladderModel.dna)){options=ladderModel;ladderModel=null;}
  const maps=[profitModel,timeModel,trendModel,volatilityModel,ladderModel].map(m=>new Map((m?.dna||[]).map(x=>[x.key,x])));
  const keys=[...new Set(maps.flatMap(m=>[...m.keys()]))];
  const dna=keys.map(k=>buildProfile(maps[0].get(k),maps[1].get(k),maps[2].get(k),maps[3].get(k),maps[4].get(k),options)).sort((a,b)=>b.samples-a.samples);
  return{version:VERSION,createdAt:new Date().toISOString(),dataPolicy:'Profit, Time, Trend, Volatility ve Kademe davranışları açıklanabilir tek profilde birleştirilir; canlı emir kararı verilmez.',totalDna:dna.length,readyDna:dna.filter(x=>x.ready).length,dna,highRisk:dna.filter(x=>x.ready&&x.risk==='YUKSEK').slice(0,20),stableProfit:dna.filter(x=>x.ready&&x.profit.character==='ISTIKRARLI_KAR_URETEN').slice(0,20)};
}
