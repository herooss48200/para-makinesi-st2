...
try {
  ...
  assert.strictEqual(ayarlar.calisilmakIstenenUsdtMiktar, 8, 'marjin 8 USDT olmalı');
  assert.strictEqual(ayarlar.mevcutKaldirac, 5, 'kaldıraç 5x korunmalı');
  assert.strictEqual(ayarlar.calisilmakIstenenUsdtMiktar * ayarlar.mevcutKaldirac, 40, 'toplam notional 40 USDT olmalı');
  ...
  console.log('✅ v6.13.5-R22.2 kasa recovery passed | 40 USDT notional | DIRECT real 0.50T/1.00T only | CONFIRMED exempt | post-close 24h no-order tracker');
} finally {
