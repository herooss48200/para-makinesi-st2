...
function openingText(pos, options = {}) {
  ...
  return `🚀 <b>ST2 ${mode} POZİSYON AÇILDI</b>\n\n` +
    ...
    (ayarlar.confirmedYuzdeselEkonomiAktif === true
      ? `Başlangıç SL ${price(pos?.sl, digits)} (-%${Number(ayarlar.sabitStopYuzdesi||2.5).toFixed(2)}) | <b>YÜZDESEL EKONOMİ</b> | +%${Number(ayarlar.confirmedYuzdeselEkonomiAktivasyonYuzde||2.5).toFixed(2)} görülene kadar erken kâr kilidi YOK | İlk kilit +%${Number(ayarlar.confirmedYuzdeselEkonomiIlkKilitYuzde||1.5).toFixed(2)} | Sonra ${Number(ayarlar.confirmedYuzdeselEkonomiAdimYuzde||0.5).toFixed(2)} puan adım / yaklaşık %${Number(ayarlar.confirmedYuzdeselEkonomiTakipMesafeYuzde||1).toFixed(2)} geriden takip\n`
...
function closingText(pos, ctx = {}) {
  ...
  return `${ctx.emoji || '🏁'} <b>${html(ctx.title || 'ST2 POZİSYON KAPANDI')}</b>\n\n` +
    ...
    `🛡️ <b>AÇILIŞ YÖNETİM PLANI</b>\n` +
    (ayarlar.confirmedYuzdeselEkonomiAktif === true
      ? `SL -%${Number(ayarlar.sabitStopYuzdesi||2.5).toFixed(2)} | YÜZDESEL EKONOMİ | +%${Number(ayarlar.confirmedYuzdeselEkonomiAktivasyonYuzde||2.5).toFixed(2)} → ilk stop +%${Number(ayarlar.confirmedYuzdeselEkonomiIlkKilitYuzde||1.5).toFixed(2)} | Sonra ${Number(ayarlar.confirmedYuzdeselEkonomiAdimYuzde||0.5).toFixed(2)} puan adımla yaklaşık %${Number(ayarlar.confirmedYuzdeselEkonomiTakipMesafeYuzde||1).toFixed(2)} geriden takip\n\n`
      : String(p.liveMode).toUpperCase() === 'SAFE_COMMISSION_BRICK_TRAIL'
      ? (p.confirmedLongLife
