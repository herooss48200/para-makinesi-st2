/**
 * AGROS v4.9.2 — UNIVERSAL EVIDENCE ENGINE
 * Strategy-agnostic historical/recent/exit/live evidence evaluation.
...
function positiveEconomics(metrics={}, minSamples=5){
  const total=num(metrics.total ?? metrics.closed);
  return total>=minSamples && num(metrics.profitFactor)>1 && num(metrics.net)>0 && num(metrics.expectancy)>0;
}
function confidenceBand(confidence){
  const c=num(confidence);
  if(c>=80)return 'GUCLU';
  if(c>=60)return 'ORTA';
  return 'DENEYSEL';
}
function evaluate({strategyType='UNKNOWN',strategyKey='',historical={},recent={},exit={},live={}}={}){
  ...
  const recentMinSamples=Math.max(1,num(ayarlar.evidenceRecentMinSamples,5));
  ...
  // Virtual research admission is based on positive economics, not a win-rate vanity threshold.
  const historicalPositive=total>=minHistorical && pf>minPf && net>minNet && exp>minExp;
  const recentPositive=ayarlar.evidenceRecent5Aktif!==false && positiveEconomics(recent,recentMinSamples);
  ...
  return {
    ...
    counts:{historical:total,recent:num(recent.total ?? recent.closed),exit:exitSamples,live:liveClosed},
    thresholds:{minHistorical,minWrInformational:minWr,minPf,minNet,minExp,minExitSamples,minConfidenceInformational:minConfidence,recentMinSamples},
    realTradingAuthorized:false
  };
}
module.exports={VERSION,evaluate,positiveEconomics,confidenceBand};
