...
function confidence(profile){
  ...
  const regimeCompleteness=Object.values(profile.context||{}).filter(v=>v&&v!=='UNKNOWN').length/7*100;
...
function evolution(profile){
  ...
  const trend=last3.n<3?'YETERSIZ_VERI':delta>0.02?'GUCLENIYOR':delta<-0.02?'ZAYIFLIYOR':'YATAY';
...
function registry(){
  ...
  return {version:VERSION,generatedAt:new Date().toISOString(),policy:{shadowOnly:true,tradeEngineChanged:false},health:base.health,profiles};
}
