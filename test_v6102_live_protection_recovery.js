...
(async()=>{
  try {
    ...
    assert.deepStrictEqual(bridge.liveRiskProfile(),{marginUsdt:4,notionalUsdt:20,leverage:5,marginType:'ISOLATED',maxActivePositions:20,protectionRequired:true});
    ...
    assert.strictEqual(r2.ok,true,'ikinci aktif pozisyon güncel 20-slot politikasında kabul edilmedi');
    ...
    // Kalan 18 slot doldurulmalı; 21. gerçek pozisyon 20/20 limitinde reddedilmelidir.
    const extraSymbols=Array.from({length:18},(_,i)=>`SLOT${String(i+3).padStart(2,'0')}USDT`);
    for (let i=0;i<extraSymbols.length;i++) {
      ...
      assert.strictEqual(rx.ok,true,`${i+3}. gerçek pozisyon 20-slot politikasında kabul edilmedi: ${rx.reason}`);
    ...
    assert.strictEqual([...state.positions.values()].filter(p=>Math.abs(Number(p.qty||0))>0).length,20,'20 gerçek slot doldurulamadı');
    const rLimit=await ex.reserveEntry({symbol:'LTCUSDT',side:'LONG',context:{...ctx,sym:'LTCUSDT',girisAnalizi:{patternId:'LIM',sonKapaliTuglaZamani:99}},client});
    assert.strictEqual(rLimit.ok,false,'21. gerçek pozisyon 20/20 limitinde reddedilmedi');
    assert(/AKTIF_POZISYON_LIMITI:20\/20/.test(rLimit.reason),`beklenmeyen limit nedeni: ${rLimit.reason}`);
    ...
    const r3=await ex.reserveEntry({symbol:'NEARUSDT',side:'LONG',context:{...ctx,sym:'NEARUSDT',girisAnalizi:{patternId:'Z',sonKapaliTuglaZamani:3}},client});
    assert.strictEqual(r3.ok,false); assert(/^GLOBAL_BLOCK:/.test(r3.reason));
    console.log('✅ v6.13.0 compatibility + current 20 real slots + independent protections + 21st-slot rejection + rollback accounting passed');
  } finally { Module._load=originalLoad; fs.rmSync(tempDir,{recursive:true,force:true}); }
})().catch(e=>{console.error('❌ v6.13.0 current 20-slot compatibility test failed:',e.stack||e);process.exitCode=1;});
