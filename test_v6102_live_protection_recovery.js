...
(async()=>{
  try {
    ...
    assert.deepStrictEqual(bridge.liveRiskProfile(),{marginUsdt:4,notionalUsdt:20,leverage:5,marginType:'ISOLATED',maxActivePositions:10,protectionRequired:true});
    ...
    assert.strictEqual(r2.ok,true,'ikinci aktif pozisyon güncel 10-slot politikasında kabul edilmedi');
    ...
    const extraSymbols=['BNBUSDT','XRPUSDT','ADAUSDT','SOLUSDT','DOGEUSDT','LINKUSDT','AVAXUSDT','DOTUSDT'];
    ...
    console.log('✅ v6.13.0 compatibility + current 10 real slots + independent protections + eleventh-slot rejection + rollback accounting passed');
  } finally { Module._load=originalLoad; fs.rmSync(tempDir,{recursive:true,force:true}); }
})().catch(e=>{console.error('❌ v6.13.0 current 10-slot compatibility test failed:',e.stack||e);process.exitCode=1;});
