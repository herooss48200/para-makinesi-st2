...
assert(revizyon.includes("const mumCacheHazir=Object.keys(h.state.yerelPusuHafizasi||{}).filter(sym=>aktifEvren.has(String(sym))).length;"), 'Mum sağlık sayacı yalnız aktif evren cache üzerinden okunmalı');
assert(revizyon.includes("const sniperCacheHazir=Object.keys(h.state.sniperMumlar||{}).filter(sym=>aktifEvren.has(String(sym))).length;"), '1m Renko ST sağlık sayacı yalnız aktif evren cache üzerinden okunmalı');
...
assert(rapor.includes('Math.min(secilen, Math.max(0, Number(veri.mumHazir || 0)))'), 'Telegram cache payını paydaya clamp etmeli');
